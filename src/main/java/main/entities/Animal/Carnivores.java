...
public class Carnivores extends Animal {
    //Magic numbers fix
    private static final double ATTACK_PROB = 30.0;
    ...
    public void animalEats(final Animal prey, final Plant plant, final Water water) {
        int entitiesEaten = 0;
        //Carnivore animals will consume other animals if there are any
        if (prey != null && prey.getMass() != ZERO_MASS) {
            setMass(getMass() + prey.getMass());
            //prey dies
            prey.setMass(ZERO_MASS);
            prey.setIsDead(true);
            entitiesEaten = 1;
        } else {
            //If there is no animal, it will eat plants OR/AND drink water
